...
if ($role === 'R01') {
    ...
    $mc = $conn->prepare("SELECT c.ClubID, c.ClubName FROM club c JOIN membership m ON c.ClubID = m.ClubID WHERE m.UserID = ? AND m.MemberStatus = 'Active'");
        ...
        <form method="POST" action="">
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px; margin-bottom:20px;">
                <div>
                    ...
                    <input type="number" name="MaxParticipants" class="form-control" style="width:100%; padding:10px; border:1px solid #ddd; border-radius:8px;" value="<?= htmlspecialchars($event['MaxParticipants']) ?>" required>
                </div>
                <div>
