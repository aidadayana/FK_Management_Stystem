...
if ($keyword !== '') {
    $where[]  = "(e.EventName LIKE ? OR e.Venue LIKE ? OR c.ClubName LIKE ?)";
    $like     = "%$keyword%";
...
$sql  = "SELECT e.*, c.ClubName FROM events e
...
$highlight = $conn->query("
    SELECT e.*, c.ClubName FROM events e
    JOIN clubs c ON e.ClubID = c.ClubID
...
<head>
    ...
    <style>
        ...
        .event-card-body h3 { font-size: 0.98rem; color: var(--text-dark); margin: 0; }
        ...
        .highlight-banner {
            ...
            box-shadow: 0 8px 25px rgba(139,30,63,0.25); gap: 20px;
        }
        .highlight-banner h2 { margin: 0 0 6px 0; font-size: 1.5rem; }
        ...
        .search-bar input, .search-bar select {
            ...
            flex: 1; min-width: 160px; color: var(--text-dark);
        }
        ...
        .btn-white { background: white; color: var(--primary-maroon); font-weight: 700; }
    </style>
</head>
<body>
...
<div class="main-content">
    ...
    <div class="highlight-banner">
        <div>
            <div style="font-size:0.72rem;text-transform:uppercase;opacity:0.7;margin-bottom:6px;">⭐ Featured Upcoming Event</div>
            <h2><?= htmlspecialchars($highlight['EventName']) ?></h2>
            <p><?= htmlspecialchars(mb_substr($highlight['Description'] ?? '', 0, 100)) ?>…</p>
        ...
        <a href="EventDetails.php?id=<?= $highlight['EventID'] ?>" class="btn btn-white">View Details →</a>
    </div>
    <?php endif; ?>
    ...
    <?php else: ?>
        ...
        <div class="event-card">
            ...
            <div class="event-card-footer">
                <a href="EventDetails.php?id=<?= $ev['EventID'] ?>" class="btn btn-primary btn-sm" style="width:100%;justify-content:center;">View Details</a>
            </div>
        </div>
        <?php endforeach; ?>
    <?php endif; ?>
